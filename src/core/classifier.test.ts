...
test("classifyQuery - W-words anywhere in query", () => {
	...
	assert.strictEqual(
		classifyQuery("which previous software did the brave browser author make"),
		"ai",
	);
...
test("classifyQuery - Question starters (is, if, can, etc.)", () => {
	...
	assert.strictEqual(
		classifyQuery("shall we proceed with this approach"),
		"ai",
	);
});
...
test("classifyQuery - Command words (summarize, explain, compare, etc.)", () => {
	...
	assert.strictEqual(
		classifyQuery("Summarize the key points of quantum computing"),
		"ai",
	);
	...
	assert.strictEqual(
		classifyQuery("denmark just had election summarize the results"),
		"ai",
	);
	assert.strictEqual(classifyQuery("latest ai news explain the impact"), "ai");
	assert.strictEqual(
		classifyQuery("python vs javascript compare performance"),
		"ai",
	);
	assert.strictEqual(
		classifyQuery("quantum computing define the basics"),
		"ai",
	);
...
test("classifyQuery - Long queries (word count >= 10)", () => {
	...
	assert.strictEqual(
		classifyQuery(
			"the best way to learn programming for absolute beginners today online",
		),
		"ai",
	);
	// More than 10 words
	assert.strictEqual(
		classifyQuery(
			"explain quantum computing in simple terms that a beginner can understand easily",
		),
		"ai",
	);
	assert.strictEqual(
		classifyQuery(
			"the best way to learn programming for beginners without any prior experience",
		),
		"ai",
	);
	...
	assert.strictEqual(
		classifyQuery("best programming language for beginners"),
		"serp",
	);
});
...
test("classifyQuery - Conjunction patterns (but/and + question word)", () => {
	...
	assert.strictEqual(
		classifyQuery("lisbon flights are cheap but how long are they"),
		"ai",
	);
	assert.strictEqual(
		classifyQuery("lisbon flights are cheap but are they long"),
		"ai",
	);

	// More "but" patterns
	assert.strictEqual(
		classifyQuery("prices are good but is this reliable"),
		"ai",
	);
	...
	// "and" patterns
	assert.strictEqual(
		classifyQuery("interesting stuff and are they available"),
		"ai",
	);
	...
	assert.strictEqual(
		classifyQuery("looks promising but has anyone tested it"),
		"ai",
	);
	assert.strictEqual(
		classifyQuery("seems popular but have people reported issues"),
		"ai",
	);
	assert.strictEqual(
		classifyQuery("tried it once but had they fixed the bug"),
		"ai",
	);
	assert.strictEqual(
		classifyQuery("it works well and may I suggest an improvement"),
		"ai",
	);
	...
	assert.strictEqual(
		classifyQuery("planning this and shall we include tests"),
		"ai",
	);
});
...
test("classifyQuery - Complex real-world examples", () => {
	...
	assert.strictEqual(
		classifyQuery(
			"flight to lisbon is 10h long, but how long is a trip to the moon",
		),
		"ai",
	); // "how" + comma
	...
	// Multi-part queries
	assert.strictEqual(
		classifyQuery("python vs javascript; which is better"),
		"ai",
	); // semicolon
	...
	assert.strictEqual(
		classifyQuery(
			"I need to understand how machine learning works and what tools to use",
		),
		"ai",
	); // "how" + "what" + 14 words
	...
	assert.strictEqual(
		classifyQuery("nice features and are they easy to use"),
		"ai",
	); // "and are"
});
